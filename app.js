...
async function savePdf(){
  ...
  try{
    ...
    const reportIso = $("fReportDate").value || '';
    ...
    for (const cat of FORM_DEF.categories){
      ...
      if (cat.type === 'visible_emissions'){
        if (isNA){
          ...
          safeSetText(form, cat.commentsField, 'N/A');
        } else {
      ...
      if (isNA){
        // uncheck everything; put N/A into the first comment cell
        qs.forEach((q, idx) => {
          safeSetCheckbox(form, q.yesField, false);
          safeSetCheckbox(form, q.noField, false);
          safeSetText(form, q.commentField, idx === 0 ? 'N/A' : '');
        });
      } else {
        qs.forEach((q, idx) => {
